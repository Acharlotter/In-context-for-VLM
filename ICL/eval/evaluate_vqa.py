...
from open_flamingo.eval.eval_model import BaseEvalModel  #调用模型的核心

from open_flamingo.eval.ok_vqa_utils import postprocess_ok_vqa_generation #没用到
...
def main():
    ...
    # Skip DDP for single GPU inference to save memory
    if args.world_size > 1:
        eval_model.init_distributed()

    # Allow few-shot eval for all models (not just OpenFlamingo)
...
def evaluate_vqa(
    ...
    question2answer_list_path = f"/hy-tmp/VQA_VQAv2/open_flamingo/eval/data/{dataset_name}/{dataset_name}_question_to_answer.json"
    question_id2answer_list_path = f"/hy-tmp/VQA_VQAv2/open_flamingo/eval/data/{dataset_name}/{dataset_name}_question_id2answer.json"
    ...
    test_dataloader = prepare_eval_samples(
        test_dataset,
        args.num_samples if args.num_samples > 0 else len(test_dataset), #num_samples为0时，加载所有数据
        args.batch_size,
        seed,
    )
    ...
    if not os.path.exists(f"{dataset_name}results_{random_uuid}.json"):
        for batch in tqdm(
            ...
            disable=args.rank != 0,
        ):#batch是一个字典，包含'image', 'question', 'question_id', 'answers'等键
            #pdb.set_trace()
            batch_demo_samples = sample_batch_demos_from_query_set(
                train_dataset, effective_num_shots, batch,
                control_signals["retrieval_type"], control_signals["clip"])
            #batch_demo_samples选取的是RS或者其他检索策略的前4个，用全局下标如np.int64(147010)来定位到v2_OpenEnded_mscoco_train2014_questions.json中的问题
            batch_images = []
            batch_text = []
            for i in range(len(batch["image"])):
                ...
                if control_signals["specification"]:
                    context_text = "Answer with a single noun or short phrase only. Do not use a sentence." + context_text
        ...
        # all gather (only for multi-GPU setups)
        if args.world_size > 1:
            ...
            # Single GPU - no need for all_gather
