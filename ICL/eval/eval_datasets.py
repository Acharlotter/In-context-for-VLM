...
class VQADataset(Dataset):
    def __init__(
        ...
        if self.dataset_name in {"vqav2"} and self.is_train == False:
            retrieval_path = "VQA_VQAv2/retrieval/retrieval_results/vqa/validation_VQAv2_RS.npy"
            self.retrieval_set = np.load(retrieval_path, allow_pickle=True).item()
        if self.dataset_name in {"ok_vqa"} and self.is_train == False:
            retrieval_path = "/hy-tmp/Qwen_VQAv2/retrieval/retrieval_results/okvqa/okvqa_validation_SQA.npy"
            self.retrieval_set = np.load(retrieval_path, allow_pickle=True).item()
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name in {"vqav2"} and self.is_train == False:
            question_id = question['question_id']
            #results["SQA_SI_4(1)"] = [i[2] for i in self.retrieval_set[question_id]["SQA_SI_4(1)"]]
            #results["SI"] = [i[2] for i in self.retrieval_set[question_id]["SI"]]
            results["RS"] = [i[2] for i in self.retrieval_set[question_id]["RS"]]
            #results["SQA_generated"] = [i[2] for i in self.retrieval_set[question_id]["SQA_generated"]]
            #results["I_SQA"] = [i[2] for i in self.retrieval_set[question_id]["I_SQA"]]
            #results["SI_Q"] = [i[2] for i in self.retrieval_set[question_id]["SI_Q"]]
            #results["Q_SI"] = [i[2] for i in self.retrieval_set[question_id]["Q_SI"]]
        ...
        if self.dataset_name in {"ok_vqa"} and self.is_train == False:
            question_id = question['question_id']
            results["SQA"] = [i[2] for i in self.retrieval_set[question_id]["SQA"]]
            #results["SQ_I"] = [i[2] for i in self.retrieval_set[question_id]["SQ_I"]]
        if self.dataset_name in {"vizwiz"} and self.is_train == False:
            question_id = question['question_id']
            results["SQ"] = [i[2] for i in self.retrieval_set[question_id]["SQ"]]
